...
public class ExecutionTimeMetric extends AbstractMetric {
     ...
     * @param phases
     *            the phases that shall be output additionally to the total execution time
    ...
    public ExecutionTimeMetric(final IFactory<AbstractLayoutProvider> layoutFactory,
            ...
            final String[] phases, final OutputStreamWriter osw) {
        super(osw, parameters, propertyHolder);
        layoutProvider = layoutFactory.create();
        this.phases = phases;
    }
    ...
    protected void warmup() throws IOException {
        ...
        warmupParameters.relativeEdgeCount = 1.5f;
        // warmupParameters.randomSeed = 1;
        System.out.println("Warmup Node Count: " + warmupNodeCount);
        KNode layoutGraph = graphGenerator.generateGraph(warmupNodeCount, warmupParameters);
    ...
    protected void doGraphSizeMeasurement(final int nodeCount) throws IOException {
        ...
        if (parameters.density > 0) {
            ...
            int edgeCount =
                    Math.round((parameters.maxOutEdgesPerNode + parameters.minOutEdgesPerNode)
                            * 0.5f * nodeCount);
            outputWriter.write(", " + Integer.toString(edgeCount));
        }
    ...
    public int getWarmupNodeCount() {
        return warmupNodeCount;
    }

    public void setWarmupNodeCount(final int warmupNodeCount) {
        this.warmupNodeCount = warmupNodeCount;
    }
