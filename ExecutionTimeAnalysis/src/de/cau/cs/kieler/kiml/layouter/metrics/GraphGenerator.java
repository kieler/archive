...
public final class GraphGenerator {
     ...
     * @param parameters
     *            user-supplied parameters affecting the graph generation.
     * @return a randomly generated graph.
     * @throws IOException
     *             if exporting the graph fails
     */
    public KNode generateGraph(final int nodeCount, final Parameters parameters) throws IOException {
        ...
        if (parameters.density > 0) {
            ...
            // Precalculate the difference between minimal and maximal number of outgoing edges per
            // node
    ...
    private KNode generateProperLayeredGraph(final int nodeCount, final Parameters parameters)
        ...
        while (remainingNodes > 0) {
            int layerSize =
                    (int) Math.round(meanLayerSize + random.nextGaussian() * meanLayerSize * 0.4);
            if (layerSize < 1) {
                layerSize = 1;
            }
     ...
     *            a graph
     * @param serialNr
     *            the serial number to append to file names
     * @throws IOException
     *             if writing the graph to a file fails
     */
    public void exportGraph(final KNode graph, final int serialNr) throws IOException {
        // Create a resource set
        ResourceSet resourceSet = new ResourceSetImpl();

        // persist layout options
     ...
     *            the target node.
     * @param parameters
     *            user-supplied parameters affecting port generation.
    ...
    private boolean createEdge(final KNode sourceNode, final KNode targetNode,
        ...
        if (!parameters.allowSelfLoops && sourceNode == targetNode || !parameters.allowCycles
                && reachable(targetNode, sourceNode)) {
    ...
    /**
     * Creates a port, adds it to the given node at a random side, and adds the given edge to the
     * new port.
     * 
     * @param node
     *            node for the new port.
     * @param edge
     *            edge to connect to the new port.
     * @param source
     *            {@code true} if the port to be created will be the source port of the edge,
     *            {@code false} if it will be its target port.
     * @param parameters
     *            user-supplied parameters affecting port generation.
     * @return a new port.
     */
    private KPort createPort(final KNode node, final KEdge edge, final boolean source,
        ...
        PortSide portSide =
                determinePortPlacement(parameters.invertedPortProb, parameters.northSouthPortProb,
                        source);
     ...
     * 
     * @param invertedSideProb
     *            the probability for ports to be placed on an odd side.
     * @param northSouthSideProb
     *            the probability for ports to be placed on the northern or southern side.
     * @param source
     *            {@code true} if the port is the source of an edge, {@code false} otherwise.
     * @return a port placement.
     */
    private PortSide determinePortPlacement(final float invertedSideProb,
            final float northSouthSideProb, final boolean source) {
    ...
    /**
     * Determine whether the given target node is reachable from the source node using a recursive
     * DFS.
     * 
     * @param node1
     *            the source node
     ...
     * @return true if the target node is reachable
     */
    private boolean reachable(final KNode node1, final KNode targetNode, final Set<KNode> visited) {
        visited.add(node1);

        for (KEdge outgoing : node1.getOutgoingEdges()) {
            KNode node2 = outgoing.getTarget();
            if (node2 == targetNode || !visited.contains(node2)
                    && reachable(node2, targetNode, visited)) {
                return true;
            }
        }
        return false;
