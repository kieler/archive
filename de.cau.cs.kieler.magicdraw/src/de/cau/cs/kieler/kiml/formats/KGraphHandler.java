...
//            KimlUtil.loadDataElements(transData.getSourceGraph());
        	KNode node = transData.getSourceGraph();
        	Iterator<EObject> iterator = node.eAllContents();
                ...
                if (eObject instanceof EMapPropertyHolder) {
                    ...
                    for (PersistentEntry persistentEntry : holder.getPersistentEntries()) {
                    	try {
							...
							System.out.println("Unhandled layout option "+persistentEntry.getKey()+" dropped");
						}
                    }
                }
            }
