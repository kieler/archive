...
function tcpsend(host, port, text)
    static t = tcpip(host,port);
    fopen(t);
    fprintf(t,text);
    fclose(t);
...
function Outputs(block)
  command = '';
  for i = 1:size(block.InputPort(1).Data)
    if i~= 1
        command = strcat(command,',');
    end
    command = strcat(command,num2str(block.InputPort(1).Data(i)));
  end;
  
  tcpsend('10.6.0.22',1234,command);
  %t = tcpip('10.6.0.22',1234);
  %fopen(t);
  %fprintf(t,command);
  %disp(command);
  %client.send(command);
...
function Terminate(block)
    tcpsend('10.6.0.22',1234,'close');
    %t = tcpip('10.6.0.22',1234);
    %fopen(t);
    %fclose(t);
%c = modelgui.ControlAnimationClient.getClient();
    %c.close();
    %modelgui.ControlAnimationClient.releaseClient();
%endfunction
